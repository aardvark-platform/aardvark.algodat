...
This document contains a minimal F# function for calculating the sun position for Earth, based on the C# code from [SunPosition.cs](https://github.com/aardvark-platform/aardvark.algodat/blob/8930ed876b4bc36fc5feeaff4d0034723b01c8c1/src/Aardvark.Physics.Sky/SunPosition.cs#L54).
...
let computeSunPosition (longitudeInDegrees: float32) (latitudeInDegrees: float32) (secondsSinceStartOfYear: float32) (year: int) : float32 * float32 * float32 =
...
let (theta, phi, distance) = computeSunPosition longitude latitude secondsSinceStartOfYear year
...
- **longitudeInDegrees**: GPS longitude in degrees, positive values for east, negative for west (range: -180 to 180)
- **latitudeInDegrees**: GPS latitude in degrees, positive values for north, negative for south (range: -90 to 90)
- **secondsSinceStartOfYear**: Number of seconds elapsed since January 1st 00:00:00 UTC of the given year
- **year**: The absolute year (e.g., 2024, 2025, etc.)

## Return Values

Returns a tuple `(theta, phi, distance)`:
- **theta**: Angle from zenith in radians (0 = zenith/directly overhead, π/2 = horizon, π = nadir)
- **phi**: Azimuth angle in radians (0 = south, π/2 = west, π = north, 3π/2 = east, 2π = south)
- **distance**: Distance to the sun in meters

## Notes

- All calculations use `float32` (single precision) as requested
- The accuracy is approximately ±1° as specified in the original implementation
- All necessary parameters are function arguments as requested
- All tables and utility functions are inlined (except for standard mathematical functions like sin, cos, etc.)
- Based on the methodology from https://www.aa.quae.nl/en/reken/zonpositie.html by Dr Louis Strous
